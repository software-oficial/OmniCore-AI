...
def validate_frontend(module_path, module_name):
    print(f"🔍 Analizando módulo: {module_name}...")
    errors = []
    ...
    if os.path.exists(required_files["index.html"]):
        with open(required_files["index.html"], "r", encoding="utf-8") as f:
            ...
            if pos_config == -1 or pos_api == -1 or pos_app == -1:
                ...
                errors.append(
                    "❌ Orden de carga incorrecto (debe ser: config -> api -> app)"
                )
        ...
        if os.path.exists(path):
            with open(path, "r", encoding="utf-8") as f:
                ...
                if content.count("{") != content.count("}"):
                    errors.append(
                        f"❌ Error de sintaxis en {js_file}: Llaves {{ }} no balanceadas"
                    )
                if content.count("(") != content.count(")"):
                    errors.append(
                        f"❌ Error de sintaxis en {js_file}: Paréntesis ( ) no balanceados"
                    )
