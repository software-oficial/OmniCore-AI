...
class WhatsappService:
    ...
    def update_setting(self, session: Session, context: CoreContext, key: str, value: str) -> ServiceResponse:
        """Updates a bot configuration setting in the database."""
        try:
            session.execute(
                text("INSERT INTO bot_settings (key, value) VALUES (:key, :value) ON CONFLICT(key) DO UPDATE SET value = :value"),
                {"key": key, "value": value}
            )
            return ServiceResponse.success_res(message=f"Setting '{key}' updated successfully.")
        except Exception as e:
            logger.error(f"Error updating setting {key}: {e}")
            return ServiceResponse.error_res(f"Update failed: {str(e)}", "SETTING_UPDATE_ERROR")
    ...
    def get_settings(self, session: Session, context: CoreContext) -> ServiceResponse:
        ...
        try:
            result = session.execute(text("SELECT * FROM bot_settings")).mappings().all()
            settings = {row['key']: row['value'] for row in result}
            return ServiceResponse.success_res(data=settings, message="Bot settings retrieved.")
        except Exception as e:
            logger.error(f"Error fetching settings: {e}")
            return ServiceResponse.error_res(f"Retrieval failed: {str(e)}", "SETTING_GET_ERROR")
    ...
    def send_text(self, session: Session, context: CoreContext, to: str, body: str, sender_type: str = 'bot') -> ServiceResponse:
        try:
            cfg = self._get_api_config()
            if not cfg["token"]:
                return ServiceResponse.error_res("API Token not configured in environment", "CONFIG_MISSING")
            ...
            return ServiceResponse.error_res(f"Failed to send message: {str(e)}", "WHATSAPP_API_ERROR")

    @command(
        name="whatsapp.upload_media",
        description="Uploads a file to Meta servers and returns the media_id for later use.",
        params_schema={"filename": "string", "mime_type": "string", "file_content": "bytes"}
    )
    def upload_media(self, session: Session, context: CoreContext, filename: str, mime_type: str, file_content: bytes) -> ServiceResponse:
        try:
            cfg = self._get_api_config()
            ...
            return ServiceResponse.success_res(data={"media_id": media_id}, message="Media uploaded successfully.")
        except Exception as e:
            logger.error(f"Error uploading media {filename}: {e}")
            return ServiceResponse.error_res(f"Media upload failed: {str(e)}", "WHATSAPP_API_ERROR")

    @command(
        name="whatsapp.send_media",
        description="Sends a previously uploaded media file (image, video, document) to a user.",
        params_schema={"to": "string", "media_id": "string", "media_type": "string", "caption": "string", "filename": "string"}
    )
    def send_media(self, session: Session, context: CoreContext, to: str, media_id: str, media_type: str, caption: str = "", filename: str = "", sender_type: str = 'bot') -> ServiceResponse:
        try:
            cfg = self._get_api_config()
            url = f"https://graph.facebook.com/v19.0/{cfg['phone_id']}/messages"
            headers = {"Authorization": f"Bearer {cfg['token']}", "Content-Type": "application/json"}
            ...
            if caption: payload[wapp_type]['caption'] = caption
            if wapp_type == 'document' and filename: payload[wapp_type]['filename'] = filename
            ...
            return ServiceResponse.success_res(data={"to": to, "status": "sent"}, message=f"Media ({wapp_type}) sent successfully.")
        except Exception as e:
            logger.error(f"Error sending WhatsApp media to {to}: {e}")
            return ServiceResponse.error_res(f"Failed to send media: {str(e)}", "WHATSAPP_API_ERROR")

    @command(
        name="whatsapp.log_message",
        description="Logs a message interaction in the business database for auditing and history.",
        params_schema={"phone_number": "string", "sender": "string", "content": "string", "message_type": "string"}
    )
    def log_message(self, session: Session, context: CoreContext, phone_number: str, sender: str, content: str, message_type: str = 'text') -> ServiceResponse:
        try:
            session.execute(text("INSERT INTO whatsapp_contacts (phone_number) VALUES (:phone) ON CONFLICT (phone_number) DO NOTHING"), {"phone": phone_number})
            session.execute(text("INSERT INTO whatsapp_conversations (phone_number) VALUES (:phone) ON CONFLICT (phone_number) DO NOTHING"), {"phone": phone_number})
            session.execute(
                text("INSERT INTO whatsapp_messages (phone_number, sender, message_type, content, timestamp) VALUES (:phone, :sender, :type, :content, CURRENT_TIMESTAMP)"),
                {"phone": phone_number, "sender": sender, "type": message_type, "content": content}
            )
    ...
    def set_human_mode(self, session: Session, context: CoreContext, phone_number: str, status: bool) -> ServiceResponse:
        try:
            session.execute(
                text("UPDATE whatsapp_conversations SET is_human_intervening = :status, last_interaction = CURRENT_TIMESTAMP WHERE phone_number = :phone"),
                {"status": status, "phone": phone_number}
            )
            return ServiceResponse.success_res(message=f"Conversation mode updated to {'Human' if status else 'Bot'}.")
        except Exception as e:
    ...
    def get_status(self, session: Session, context: CoreContext, phone_number: str) -> ServiceResponse:
        try:
            res = session.execute(
                text("SELECT is_human_intervening, current_menu FROM whatsapp_conversations WHERE phone_number = :phone"),
                {"phone": phone_number}
            ).mappings().first()
            ...
            return ServiceResponse.success_res(data=dict(res), message="Conversation status retrieved.")
        except Exception as e:
    ...
    def handle_bot_flow(self, session: Session, context: CoreContext, phone_number: str, text: str) -> ServiceResponse:
        try:
            text_clean = text.strip().lower()
            
            # 1. Resolve Conversation State
            conv_res = session.execute(
                text("SELECT current_menu, is_human_intervening FROM whatsapp_conversations WHERE phone_number = :phone"),
                {"phone": phone_number}
            ).mappings().first()
            
            if not conv_res:
                session.execute(text("INSERT INTO whatsapp_conversations (phone_number, current_menu) VALUES (:phone, 'main')"), {"phone": phone_number})
                conv_res = {"current_menu": "main", "is_human_intervening": False}

            if conv_res['is_human_intervening']:
                return ServiceResponse.success_res(message="Conversation is in HUMAN mode. Bot will not respond.")

            # 2. Global Shortcuts
            if text_clean in ["/start", "hola", "buenas", "menu", "opciones", "volver", "menú"]:
                return self._send_main_menu(session, phone_number)

            # 3. Menu Navigation
            current_menu_name = conv_res['current_menu']
            all_menus = session.execute(text("SELECT * FROM whatsapp_menus")).mappings().all()
                ...
                for idx, opt in enumerate(options, 1):
                    if text_clean == str(idx) or text_clean == opt['value'].lower():
                        if opt['action_type'] == 'NAVIGATE':
                            session.execute(
                                text("UPDATE whatsapp_conversations SET current_menu = :menu, last_interaction = CURRENT_TIMESTAMP WHERE phone_number = :phone"),
                        ...
                        if opt['action_type'] == 'COMMAND':
                            return ServiceResponse.success_res(
                                data={"action": "EXECUTE_COMMAND", "command": opt['command_name']},
            ...
            # 5. Fallback
            fallback_msg = session.execute(
                text("SELECT value FROM bot_settings WHERE key = 'fallback_message'")
            ).scalar() or "No he entendido ese mensaje. Escribe 'menu' para ver las opciones."
            
            return ServiceResponse.success_res(message=fallback_msg)

        except Exception as e:
            logger.error(f"Error in handle_bot_flow for {phone_number}: {e}")
            return ServiceResponse.error_res(f"Bot flow error: {str(e)}", "BOT_FLOW_ERROR")

    def _send_main_menu(self, session: Session, phone_number: str) -> ServiceResponse:
        session.execute(text("UPDATE whatsapp_conversations SET current_menu = 'main' WHERE phone_number = :phone"), {"phone": phone_number})
    ...
    def _get_menu_response(self, session: Session, menu_name: str) -> ServiceResponse:
        ...
        options = session.execute(
            text("SELECT label FROM whatsapp_menu_options mo JOIN whatsapp_menus wm ON mo.menu_id = wm.menu_id WHERE wm.menu_name = :name ORDER BY mo.sort_order"),
            {"name": menu_name}
        ).mappings().all()
