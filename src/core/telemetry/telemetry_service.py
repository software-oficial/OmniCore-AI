...
class TelemetryService:
    ...
    def track_request(self, agent_id: str, app_id: str, command: str, duration: float, success: bool):
        """
        ...
        try:
            ...
            timestamp = time.time()
            pipe.zadd(f"{self.METRIC_PREFIX}latency:{command}", {f"{timestamp}:{duration}": duration})
    ...
    def get_realtime_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get active counts
            active_agents = len(cache_manager.client.keys(f"{self.METRIC_PREFIX}active_agent:*"))
            active_apps = len(cache_manager.client.keys(f"{self.METRIC_PREFIX}active_app:*"))
            
            # Get total requests
            total = int(cache_manager.client.get(f"{self.METRIC_PREFIX}total_requests") or 0)
            ...
            return {
                ...
                "status": {
                    "success": int(cache_manager.client.get(f"{self.METRIC_PREFIX}status:success") or 0),
                    "failure": int(cache_manager.client.get(f"{self.METRIC_PREFIX}status:failure") or 0),
                }
            }
        except Exception as e:
