...
class SalesService:
    ...
    def create_employee(self, session: Session, context: CoreContext, username: str, password: str, role: str = "employee") -> ServiceResponse:
        ...
        try:
            ...
            return ServiceResponse.success_res(message=f"Employee {username} created successfully with role {role}.")
        except Exception as e:
            logger.error(f"Error creating employee {username}: {e}")
            return ServiceResponse.error_res(f"User creation failed: {str(e)}", "USER_CREATE_ERROR")
    ...
    def change_role(self, session: Session, context: CoreContext, username: str, new_role: str) -> ServiceResponse:
        """Updates user role."""
        try:
            session.execute(
            ...
            return ServiceResponse.success_res(message=f"Role for {username} updated to {new_role}.")
        except Exception as e:
            logger.error(f"Error updating role for {username}: {e}")
            return ServiceResponse.error_res(f"Update failed: {str(e)}", "USER_ROLE_ERROR")
    ...
    def report_daily(self, session: Session, context: CoreContext, date: str) -> ServiceResponse:
        """Generates a daily financial report."""
        try:
            # Total Sales
            total_res = session.execute(
                text("SELECT SUM(total_amount) as total FROM sales WHERE DATE(created_at) = :date"),
                {"date": date}
            ).scalar()
            
            # Split by method
            split_res = session.execute(
                text("SELECT payment_method, SUM(total_amount) as sum FROM sales WHERE DATE(created_at) = :date GROUP BY payment_method"),
                {"date": date}
            ).mappings().all()
            ...
            return ServiceResponse.error_res(f"Report failure: {str(e)}", "REPORT_ERROR")
    ...
    @command(
        ...
        params_schema={"payment_id": "string", "external_reference": "string", "status": "string"}
    )
    def handle_mp_webhook(self, session: Session, context: CoreContext, payment_id: str, external_reference: str, status: str) -> ServiceResponse:
        """
        ...
        try:
            if status != "approved":
                return ServiceResponse.success_res(message=f"Payment {payment_id} status is {status}. No action taken.")
            ...
            if not sale:
                return ServiceResponse.error_res("Pending sale not found for this reference", "SALE_NOT_FOUND")
            
            # 2. Mark sale as completed
            session.execute(
                text("UPDATE sales SET status = 'COMPLETED', payment_method = 'MercadoPago' WHERE id = :id"),
                {"id": external_reference}
            )
            
            # 3. Deduct stock for all items in this sale
            items = session.execute(
                text("SELECT product_code, quantity FROM sale_items WHERE sale_id = :id"),
                {"id": external_reference}
            ).mappings().all()
            ...
            logger.error(f"Error handling MP webhook for sale {external_reference}: {e}")
            return ServiceResponse.error_res(f"Webhook processing failed: {str(e)}", "WEBHOOK_ERROR")
    ...
    def grant_permission(self, session: Session, context: CoreContext, username: str, permission_key: str) -> ServiceResponse:
        """Grants a specific permission key to a user."""
        try:
            # First resolve user_id
            user = session.execute(text("SELECT id FROM users WHERE username = :u"), {"u": username}).mappings().first()
            if not user:
                return ServiceResponse.error_res("User not found", "USER_NOT_FOUND")
            
            # Grant permission
            session.execute(
                text("INSERT INTO user_permissions (user_id, permission_key) VALUES (:uid, :perm) ON CONFLICT DO NOTHING"),
                {"uid": user['id'], "perm": permission_key}
            )
            return ServiceResponse.success_res(message=f"Permission {permission_key} granted to {username}.")
        except Exception as e:
    ...
    def revoke_permission(self, session: Session, context: CoreContext, username: str, permission_key: str) -> ServiceResponse:
        ...
        try:
            ...
            session.execute(
                text("DELETE FROM user_permissions WHERE user_id = :uid AND permission_key = :perm"),
                {"uid": user['id'], "perm": permission_key}
            )
            return ServiceResponse.success_res(message=f"Permission {permission_key} revoked from {username}.")
        except Exception as e:
            logger.error(f"Error revoking permission from {username}: {e}")
            return ServiceResponse.error_res(f"Revocation failed: {str(e)}", "PERM_ERROR")
    ...
    def open_cash_box(self, session: Session, context: CoreContext, monto_inicial: float) -> ServiceResponse:
        ...
        try:
            ...
            return ServiceResponse.success_res(message=f"Cash box opened successfully with ${monto_inicial}.")
        except Exception as e:
    ...
    def close_cash_box(self, session: Session, context: CoreContext, monto_real: float) -> ServiceResponse:
    ...
    def get_cash_box_status(self, session: Session, context: CoreContext) -> ServiceResponse:
        """Retorna el estado actual de la caja."""
        try:
            query = text("SELECT * FROM cash_box WHERE id = 1")
            status = session.execute(query).mappings().first()
            return ServiceResponse.success_res(data=dict(status) if status else None, message="Cash box status retrieved.")
        except Exception as e:
    ...
    def create_new_sale_session(self, session: Session, context: CoreContext) -> ServiceResponse:
        """Initializes a sale session."""
        return ServiceResponse.success_res(message="Sale session initialized. Use venta.add to add products.")
    ...
    def add_to_sale(self, session: Session, context: CoreContext, codigo: str, cantidad: int) -> ServiceResponse:
    ...
    @command(
        ...
        params_schema={"cliente": "string", "items": "list[dict]", "metodo_pago": "string", "paga_con": "float", "alias": "string"}
    )
    def process_sale(self, session: Session, context: CoreContext, cliente: str, items: List[Dict[str, Any]], payment_method: str, paga_con: float = 0.0, alias: Optional[str] = None) -> ServiceResponse:
        """
        ...
        try:
            ...
            for item in items:
                ...
                qty = item['quantity']
                product_res = stock_service.get_product(session, context, code=product_code)
            ...
            if payment_method == "Transferencia" and alias:
                alias_query = text("SELECT acumulado, limite FROM aliases WHERE nombre = :nombre")
                alias_data = session.execute(alias_query, {"nombre": alias}).mappings().first()
                if not alias_data:
                    return ServiceResponse.error_res("Alias not registered.", "ALIAS_NOT_FOUND")
                if (float(alias_data['acumulado'] or 0)) + total_amount > float(alias_data['limite']):
                    return ServiceResponse.error_res(f"Limit exceeded for alias {alias}.", "ALIAS_LIMIT_EXCEEDED")

            # 3. Cash Payment Validation
            vuelto = 0.0
            if payment_method == "Efectivo":
                if paga_con < total_amount:
                    return ServiceResponse.error_res("Insufficient cash amount.", "CASH_INSUFFICIENT")
            ...
            sale_id = session.execute(sale_query, {
                "name": cliente, "total": total_amount, "method": payment_method, "paga_con": paga_con, "vuelto": vuelto
            }).scalar()
            ...
            # 5. Update Cash Box
            col = "ventas_efectivo" if payment_method == "Efectivo" else "ventas_digital"
            cash_query = text(f"UPDATE cash_box SET {col} = {col} + :total WHERE id = 1")
            session.execute(cash_query, {"total": total_amount})

            # 6. Update Alias Accumulator
            if payment_method == "Transferencia" and alias:
                alias_update = text("UPDATE aliases SET acumulado = acumulado + :total WHERE nombre = :nombre")
                session.execute(alias_update, {"total": total_amount, "nombre": alias})

            # 7. Deduct Stock (Ledger)
            for pi in processed_items:
                stock_service.update_stock(session, context, code=pi['product_code'], quantity=-pi['qty'], reason=f"SALE_{sale_id}")
    ...
    def cancel_sale(self, session: Session, context: CoreContext, sale_id: int) -> ServiceResponse:
        """Cancels a sale."""
        try:
            sale = session.execute(text("SELECT status FROM sales WHERE id = :id"), {"id": sale_id}).mappings().first()
            if not sale or sale['status'] == 'COMPLETED':
                return ServiceResponse.error_res("Sale cannot be cancelled.", "SALE_INVALID")
            
            session.execute(text("UPDATE sales SET status = 'CANCELLED' WHERE id = :id"), {"id": sale_id})
            return ServiceResponse.success_res(message="Sale cancelled successfully.")
        except Exception as e:
            logger.error(f"Error cancelling sale {sale_id}: {e}")
            return ServiceResponse.error_res(f"Internal error: {str(e)}", "SALE_CANCEL_ERROR")
    ...
    def create_payment_link(self, session: Session, context: CoreContext, codigo: str, cantidad: int, cliente: str) -> ServiceResponse:
        ...
        try:
            ...
            if not product_res.success: return product_res
            
            total = float(product_res.data['price']) * cantidad
            
            sale_query = text("INSERT INTO sales (client_name, total_amount, status, payment_method) VALUES (:name, :total, 'PENDING', 'MercadoPago') RETURNING id")
            sale_id = session.execute(sale_query, {"name": cliente, "total": total}).scalar()
            
            session.execute(text("INSERT INTO sale_items (sale_id, product_code, quantity, unit_price, subtotal) VALUES (:sid, :code, :qty, :price, :sub)"),
                            {"sid": sale_id, "code": codigo, "qty": cantidad, "price": product_res.data['price'], "sub": total})

            # 2. Generate MP Link
            from src.core.system_service import system_service
            token_res = system_service.get_setting("mp_access_token", session=session, context=context)
            mp_token = token_res.get("value")
            
            if not mp_token:
                return ServiceResponse.error_res("MP Token not configured in system settings.", "CONFIG_MISSING")
            ...
            return ServiceResponse.error_res(f"Internal la error: {str(e)}", "MP_LINK_ERROR")
    ...
    def add_alias(self, session: Session, context: CoreContext, nombre: str, limite: float) -> ServiceResponse:
        """Creates a new alias."""
        try:
            import uuid
            alias_id = str(uuid.uuid4())[:8]
            session.execute(
                text("INSERT INTO aliases (id, nombre, limite, acumulado) VALUES (:id, :nombre, :limite, 0)"),
                {"id": alias_id, "nombre": nombre, "limite": limite}
            )
            return ServiceResponse.success_res(message=f"Alias {nombre} created successfully.")
        except Exception as e:
            logger.error(f"Error adding alias: {e}")
            return ServiceResponse.error_res(f"Internal error: {str(e)}", "ALIAS_ADD_ERROR")
    ...
    def list_aliases(self, session: Session, context: CoreContext) -> ServiceResponse:
        ...
        try:
            ...
            return ServiceResponse.success_res(data=[dict(a) for a in result], message="Aliases listed successfully.")
        except Exception as e:
            logger.error(f"Error listing aliases: {e}")
            return ServiceResponse.error_res(f"Internal error: {str(e)}", "ALIAS_LIST_ERROR")
    ...
    def delete_alias(self, session: Session, context: CoreContext, alias_id: str) -> ServiceResponse:
        """Deletes an alias."""
        try:
            session.execute(text("DELETE FROM aliases WHERE id = :id"), {"id": alias_id})
            return ServiceResponse.success_res(message=f"Alias {alias_id} deleted successfully.")
        except Exception as e:
            logger.error(f"Error deleting alias: {e}")
            return ServiceResponse.error_res(f"Internal error: {str(e)}", "ALIAS_DELETE_ERROR")
