...
class DynamicDbManager:
    ...
    def _create_engine(
        self, db_config: Dict[str, Any], pool_size: int = 5, max_overflow: int = 10
    ):
    ...
    def report_failure(self, app_id: str):
        ...
        if state["failures"] >= self.FAILURE_THRESHOLD:
            logger.error(
                f"🚨 Circuit OPENED for App {app_id} due to repeated failures."
            )
            state["status"] = "OPEN"
    ...
    def _check_circuit(self, app_id: str):
        ...
        if state["status"] == "OPEN":
            if time.time() - state["last_failure"] > self.RECOVERY_TIMEOUT:
                logger.info(
                    f"🌓 Circuit HALF-OPEN for App {app_id}. Testing connection..."
                )
                return

            raise InfrastructureException(
                message="Connection to business database is temporarily suspended. Please try again in a few minutes.",
    ...
    async def get_session(
        ...
        async with self._semaphore:
            ...
            if app_id not in self._engines:
                ...
                try:
                    logger.info(
                        f"Initializing new DB pool for App {app_id} [Tier: {tier}]"
                    )
                    ...
                    raise InfrastructureException(
                        f"Could not initialize DB pool: {str(e)}", "DB_INIT_FAILED"
                    )
            ...
            try:
                yield session
                if (
                    app_id in self._circuit_breakers
                    and self._circuit_breakers[app_id]["status"] != "CLOSED"
                ):
                    logger.info(
                        f"✅ Circuit CLOSED for App {app_id}. Connection restored."
                    )
                    self._circuit_breakers[app_id] = {
                        "failures": 0,
                        "last_failure": 0,
    ...
    def evict_idle_pools(self, max_idle_seconds: int = 1800):
        ...
        to_evict = [
            app_id
            for app_id, last_time in self._last_accessed.items()
