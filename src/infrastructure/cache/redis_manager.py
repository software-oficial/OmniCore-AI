...
class RedisManager:
    ...
    def _connect(self):
        ...
        try:
            ...
            logger.error(
                f"⚠️ Redis connection failed: {e}. System entering Degraded Mode."
            )
            self.client = None
    ...
    def cache_learning_error(
        self, agent_id: str, func_name: str, error_pattern: str, correction: str
    ):
    ...
    def get_learning_correction(
        self, agent_id: str, func_name: str, error_pattern: str
    ) -> Optional[str]:
        ...
        try:
            return self.client.hget(
                f"learn:pattern:{agent_id}:{func_name}", error_pattern
            )
        except Exception as e:
