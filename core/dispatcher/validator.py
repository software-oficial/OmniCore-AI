...
class RequestValidator:
    ...
    @staticmethod
    def validate(command_name: str, params: Dict[str, Any], cmd_schema: Dict[str, Any]) -> Tuple[bool, Optional[ServiceResponse], Dict[str, Any]]:
        """
        ...
        if command_name and params and isinstance(cmd_schema, dict):
            ...
            if len(filtered_params) != len(params):
                logger.warning(f"⚠️ Mass Assignment Attempt blocked for {command_name}. Dropped keys: {set(params.keys()) - set(cmd_schema.keys())}")
