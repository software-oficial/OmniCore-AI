...
async def verify_dev_access(authorization: str = Header(None)):
    ...
    if authorization != master_key and authorization != f"Bearer {master_key}":
        raise HTTPException(status_code=403, detail="Forbidden: Master Developer Key required.")
    return True
...
async def onboard_client(payload: Dict[str, Any]):
    ...
    try:
        ...
        return {"success": True, "app_id": app_id, "message": f"Client {payload['app_name']} onboarded."}
...
async def deploy_client_schema(app_id: str):
    ...
    async with db_manager.get_session(app_id, app_info['db_config'], app_info['tier']) as session:
...
async def generate_client_token(app_id: str, token_name: str, user_id: str):
    ...
    agent_id = res[0]
    response = auth_service.create_api_token(user_id=user_id, agent_id=agent_id, token_name=token_name)
...
async def create_plan(payload: Dict[str, Any]):
    ...
    try:
        ...
        return {"success": True, "message": f"Plan {payload['tier_name']} created successfully."}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def list_plans():
    ...
    from infra.db.core_db_manager import core_db_manager
    plans = core_db_manager.execute_raw("SELECT tier_name, level FROM governance_tiers ORDER BY level DESC").mappings().all()
...
async def map_command_to_plan(payload: Dict[str, Any]):
    ...
    try:
        ...
        return {"success": True, "message": f"Command {payload['command_name']} now requires {payload['min_tier']}."}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def list_command_requirements():
    ...
    from infra.db.core_db_manager import core_db_manager
    reqs = core_db_manager.execute_raw("SELECT command_name, min_tier FROM governance_commands").mappings().all()
